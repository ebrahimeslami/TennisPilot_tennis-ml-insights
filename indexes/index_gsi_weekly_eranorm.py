...
def era_normalize(df):
    """Normalize GSI by decade baseline (e.g., 1990s, 2000s, 2010s)."""
    df["decade"] = (df["iso_year"] // 10) * 10
    decade_stats = (
        df.groupby("decade")["GSI"]
          .agg(["mean","std"])
          .rename(columns={"mean":"era_mean","std":"era_std"})
          .reset_index()
    )
    df = df.merge(decade_stats, on="decade", how="left")
    df["GSI_eranorm"] = (df["GSI"] - df["era_mean"]) / (df["era_std"] + 1e-6)
    df["GSI_eranorm"] = df["GSI_eranorm"].clip(-2.5, 2.5)  # bound outliers
    return df
    ...
    agg = (
        df.groupby(keys, observed=True)
          .agg(player_name=("player_name","last"),
               avg_GSI=("GSI_eranorm","mean"),
               matches=("GSI_eranorm","count"))
          .reset_index()
    )
    return agg
...
def summarize_players(df):
    summary = (
        df.groupby("player_id", observed=True)
          .agg(
              ...
              avg_GSI=("GSI_eranorm","mean"),
              peak_GSI=("GSI_eranorm","max"),
              gsi_90th=("GSI_eranorm", lambda x: np.nanpercentile(x,90)),
              variance_mean=("variance","mean"),
              first_year=("iso_year","min"),
              last_year=("iso_year","max"),
              decade_mode=("decade", lambda x: x.mode()[0] if not x.mode().empty else np.nan)
          )
          .reset_index()
    )
    ...
    summary["Greatness_Stability_EraAdj"] = (
        0.55*summary["gsi_90th"] +
        0.30*summary["stability_score"] +
        0.15*summary["avg_GSI"]
    )
    return summary.sort_values("Greatness_Stability_EraAdj", ascending=False)

# ==================================================
# Main
# ==================================================
def main():
    parser = argparse.ArgumentParser(description="Compute Greatness Stability Index (GSI) with Era Normalization")
    ...
    print("[INFO] Generating player summary...")
